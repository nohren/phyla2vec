...
Phyla2Vec is an exploratory attempt to compress an entire 16S microbiome sample from thousands of raw 150 bp reads into a single 128 dimensional embedding while preserving its phylogenetic relationship to other samples. The model randomly subsamples reads from each sample, encodes nucleotide sequences with a transformer, pools them into read level representations, contextualizes those reads with a second transformer, and pools again to produce one sample level embedding at 128 dimensions.

The model is trained through UniFrac distance supervision: pairwise Euclidean distances between learned sample embeddings are optimized to approximate the corresponding unweighted UniFrac distances. Repeated stochastic rarefaction exposes the model to different views of each microbial community while keeping the large hierarchical tensors within GPU memory. The project began as an attempt to learn representations invariant to 16S variable region, but limitations in the available data prevented those claims from being established.  We found that most data was only collected in the V4 region. So we settled on collection differences instead. The result is best understood as an ambitious experiment in phylogeny supervised, hierarchical compression of raw 16S reads, developed following discussions with the UCSD Knight Lab.
