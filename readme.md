...
<!-- This project develops a generalized phylogeny-aware encoding mechanism for turning microbiome samples into embeddings, aimed at unifying 16S rRNA samples with different collection protocols and resulting batch effects. We use studies and their different batch effects to demonstrate slight out-of-distribution offsets that make cross-study comparisons noisy, limiting the effective size of training data for health classification tasks.
...
Phyla2Vec is an exploratory attempt to compress an entire 16S microbiome sample from thousands of raw 150 bp reads into a single 128 dimensional embedding while preserving its phylogenetic relationship to other samples. The model randomly subsamples reads from each sample, encodes nucleotide sequences with a transformer, pools them into read level representations, contextualizes those reads with a second transformer, and pools again to produce one sample level embedding at 128 dimensions.  The model vocabulary is literally five tokens: A, C, G, T, and N as a catch-all.

The model is trained through UniFrac distance supervision: pairwise Euclidean distances between learned sample embeddings are optimized to approximate the corresponding unweighted UniFrac distances. Repeated stochastic rarefaction exposes the model to different views of each microbial community while keeping the large hierarchical tensors within GPU memory. The project began as an attempt to learn representations invariant to the sequenced 16S variable region, but most of the available data covered only the V4 region, making that hypothesis impossible to test. We therefore redirected the experiment toward cross study collection differences, although the resulting invariance was not established conclusively. Phyla2Vec is best understood as an ambitious experiment in phylogeny supervised, hierarchical compression of raw 16S reads.
